...
def build_return_xlsx(rows: list[dict], today: str | None = None) -> bytes:
    ...
    Each row dict needs: awb_id, po_number, pharmacy_name, phone, address, origin, reject_pcs.
    """
    ...
    for a in rows:
        ...
        row = {
            ...
            "bundle_information.requested_piece_tracking_numbers": return_piece_trid(a["po_number"]),
...
def _upload_row(service_code: str, awb: dict, trid: str, trids: list[str], today: str) -> dict:
    ...
    if awb["is_return"]:
        ...
        row.update({
            "from.name": wh["name"], "from.phone_number": wh["phone"],
            "from.address.address1": _with_hub(wh["address1"], awb.get("hub_name")),
            "from.address.country": wh["country"],
            "to.name": awb["pharmacy_name"], "to.phone_number": awb["phone"],
            "to.address.address1": awb["address"],
