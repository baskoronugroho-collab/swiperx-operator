...
STAGES = ("pending_ack", "acknowledged", "tids_sent")
...
def _shape(row: dict) -> dict:
    for k in ("rejected_at", "acknowledged_at", "tids_sent_at"):
...
async def export_csv(_: dict = Depends(ops_roles)):
    ...
    w.writerow([
        ...
        "stage", "acknowledged_at", "acknowledged_by", "return_tids", "tids_sent_at", "tids_sent_by",
    ])
    for r in rows:
        s = _shape(r)
        w.writerow([
            s["id"], s["original_awb_id"], s["pharmacy_name"] or "", s["city"] or "",
            s["return_type"], s["rejected_at"] or "", s["stage"], s["acknowledged_at"] or "",
            s["acknowledged_by_email"] or "", s["return_tids"] or "", s["tids_sent_at"] or "",
            s["tids_sent_by_email"] or "",
        ])
    return Response(
